...
VERSION   ?= $(call get_gh_version,gotify/server)
CLIVERSION ?= $(call get_gh_version,gotify/cli,CLIVERSION)
...
LABELFLAGS ?= \
	...
	--label org.opencontainers.image.version-cli=$(CLIVERSION) \
...
BUILDFLAGS ?= \
	...
	--build-arg CLIARCH=$(call get_cli_arch) \
	--build-arg CLIVERSION=$(if $(CLIVERSION),$(CLIVERSION),$(error CLIVERSION is not defined)) \
...
OTHERFLAGS := \
	...
	-e PGID=$(PGID) \
	-e PUID=$(PUID) \
	# -e GOTIFY_CONFIG=/etc/gotify/gotify.yml \
	# -e GOTIFY_DATA=/gotify/data \
	# -e GOTIFY_DEFAULTUSER_NAME=admin \
	# -e GOTIFY_DEFAULTUSER_PASS=insecurebydefault \
	# -e TZ=Asia/Kolkata \
	#
...
get_config: ## get default config from github
	curl \
		...
		-SL https://raw.githubusercontent.com/gotify/server/master/config.example.yml
...
SRC_ARCH_MAP := \
	...
	'armhf'  ) echo -n 'linux-arm-7'   ;; \
	'armv7l' ) echo -n 'linux-arm-7'   ;; \
...
CLI_ARCH_MAP := \
    ...
    #
define get_cli_arch
$(shell case "$(ARCH)" in $(CLI_ARCH_MAP) esac)
endef
# -- }}}
